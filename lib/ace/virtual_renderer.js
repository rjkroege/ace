...
(function() {
    ...
    **/
    ...
    // What if the region isn't present.
    this.screenToTextCoordinates = function(x, y) {
        console.info('x, y: ' + x + ' ' + y);
        var canvasPos = this.scroller.getBoundingClientRect();


        // Loop forwards while y is not in a bounding box.
        var r, n, cr;
        for (r = 0, n = this.$textLayer.element.firstChild, cr = n.getBoundingClientRect();
                n != null && y < cr.top && y > cr.bottom;
                r++, n = n.nextSibling, cr = n.getBoundingClientRect());

        // At this point, r is the row. I hope.
        // We can find the box. But the offset inside of a text node is very difficult?
        // Without constructing a range. Which we can do to complete the search.
        // Make a sequence of ranges.
        var c;
        for (c = 0; n = n.firstChild; /* not sure */; n = n.nextSibling) {
            switch (n.type) {
            case 
            }
        }
